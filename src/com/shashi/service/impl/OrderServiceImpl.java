...
public class OrderServiceImpl implements OrderService {
	...
	public boolean addOrder(OrderBean order) {
		...
		try {
			...
			ps.setInt(5, 0);
			Timestamp orderTime = (order.getTime() != null) ? order.getTime() : new Timestamp(System.currentTimeMillis());
	...
	public String shipNow(String orderId, String prodId) {
	 ...
	 // Assuming you have a method to get all shipped orders from the database
	 public List<OrderDetails> getAllShippedOrders() {
		    List<OrderDetails> shippedOrders = new ArrayList<>();
		    ...
		    try {
		        // Establish a database connection (implement your DBUtil class accordingly)
		        con = DBUtil.provideConnection();

		        // Prepare the SQL query to get shipped orders
		        String query = "SELECT * FROM orders WHERE shipped = 1";
		        ps = con.prepareStatement(query);

		        // Execute the query
		        rs = ps.executeQuery();

		        // Process the result set and populate the list of shipped orders
		        while (rs.next()) {
		            OrderDetails order = new OrderDetails();
		            order.setOrderId(rs.getString("orderid"));
		            order.setTime(rs.getTimestamp("time"));
	    ...
	    // Assuming you have a method to update the order status to "DELIVERED" in the database
	    public void updateOrderStatusToDelivered(String orderId) {
	        ...
	        try {
	            ...
	            // Prepare the SQL query to update order status
	            String query = "UPDATE orders SET shipped = 2 WHERE orderid = ?";
	            ps = con.prepareStatement(query);
	            ps.setString(1, orderId);
	            ...
	            if (rowsUpdated > 0) {
	                System.out.println("Order status updated to DELIVERED for order ID: " + orderId);
	            } else {

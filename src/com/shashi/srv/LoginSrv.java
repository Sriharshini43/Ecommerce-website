...
public class LoginSrv extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userType != null && userType.equals("admin")) {  // Login as Admin
            if (password.equals("admin8") && userName.equals("admin@gmail.com")) {
                // valid
            ...
            if (action != null && action.equals("forgotPassword")) {
                ...
                if (udao.checkIfEmailExistsInDatabase(email)) {
                    // If email exists, redirect to newPassword.jsp
                	request.setAttribute("userEmail", email); 
                    RequestDispatcher rd = request.getRequestDispatcher("newPassword.jsp");
                    rd.forward(request, response);
                } else {
                    // If email does not exist, show an error message
                    RequestDispatcher rd = request.getRequestDispatcher("forgotPassword.jsp?message=Email not exists");
                    rd.forward(request, response);
                }
            } else {
